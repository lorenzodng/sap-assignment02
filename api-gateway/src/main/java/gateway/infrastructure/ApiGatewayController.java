...
public class ApiGatewayController {
    ...
    private void createShipment(RoutingContext ctx) {
        ...
        request.sendBuffer(Buffer.buffer(ctx.body().asString()))
                .onSuccess(response -> {
                    ...
                    if (response.statusCode() >= 500) {
                        ...
                        requestCircuitBreaker.get().onSuccess(0, java.util.concurrent.TimeUnit.NANOSECONDS);
                    }
                    ctx.response().setStatusCode(response.statusCode()).end(response.bodyAsString());
                })
                .onFailure(err -> {
                    metrics.incrementRequest("/shipments", "POST", 500);
                    requestCircuitBreaker.get().onError(0, java.util.concurrent.TimeUnit.NANOSECONDS, err);
        ...
        request.send()
                .onSuccess(response -> {
                    ...
                    if (response.statusCode() >= 500) {
                        ...
                        deliveryCircuitBreaker.get().onSuccess(0, java.util.concurrent.TimeUnit.NANOSECONDS);
                    }
                ...
                .onFailure(err -> {
                    ...
                    deliveryCircuitBreaker.get().onError(0, java.util.concurrent.TimeUnit.NANOSECONDS, err);
                    ctx.response().setStatusCode(500).end("Error forwarding request");
                });
    }
